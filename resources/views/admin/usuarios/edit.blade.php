...
@section('content')
<div class="container">
    <h1 class="mb-4 text-primary">Editar Usuário</h1>
    ...
    <form action="{{ route('admin.usuarios.update', $usuario->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="perfil" id="perfil" class="form-select" required>
                ...
                <option value="p4" {{ $usuario->perfil === 'p4' ? 'selected' : '' }}>P4</option>
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="opm_id" id="opm_id" class="form-select" required>
                @foreach($opms as $opm)
                    <option value="{{ $opm->id }}" {{ $usuario->opm_id == $opm->id ? 'selected' : '' }}>{{ $opm->sigla }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="permitido" id="permitido" class="form-select" required>
                <option value="1" {{ $usuario->permitido ? 'selected' : '' }}>Permitido</option>
                <option value="0" {{ !$usuario->permitido ? 'selected' : '' }}>Bloqueado</option>
            </select>
        </div>
